...
func GetChallengerSignature(client *ethclient.Client, challengerRegistry *ChallengerRegistry.ChallengerRegistry, challengerAddress common.Address, challengerPrivateKey *ecdsa.PrivateKey, operatorAddress common.Address, salt [32]byte, expiry *big.Int) []byte {
	...
	signature, err := crypto.Sign(digestHash[:], challengerPrivateKey)
	op_common.CheckError(err, "Signing the digest hash failed")
